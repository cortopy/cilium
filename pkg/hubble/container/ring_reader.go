...
// position. Returns ErrConcurrentWrite if there are no older entries.
func (r *RingReader) Previous() (*v1.Event, error) {
	// We only expect ErrConcurrentWrite to be returned when reading backwards,
...
// Returns ErrEndOfBuffer if there are no more newer entries. May return
// ErrConcurrentRead if the writer overtook this RingReader.
func (r *RingReader) Next() (*v1.Event, error) {
	// There are two possible errors returned by read():
	//
	// ErrEndOfBuffer (reader ahead of writer): We have read past the writer.
	/// In this case, we want to return nil and don't bump the index, as we have
	// read all existing values that exist now.
	// ErrConcurrentWrite (writer ahead of reader): The writer has already
